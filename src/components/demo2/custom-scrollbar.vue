<template>
    <div class="scrollbar_wrap-container"
         ref="wrap"
        :style="wrapStyle"
        :class="wrapClass">
        <!-- 包装内容的容器，该容器用于呈现滚动条，利用神奇的margin将其隐藏 -->
        <div class="scrollbar_wrap-content"
            @scroll="scrolling"
            ref="view">
            <div class="scrollbar_view-box"
                :class="viewClass"
                :style="viewStyle">
                <slot></slot>
            </div>
        </div>
        <bar :movePercentage="moveY"
             :barSize="verticalBarHeight"/>
        <bar :vertical="false"
...
export default {
    ...
    mounted () {
        ...
        this.$refs.wrap && on(this.$refs.wrap, 'resize', lazyHandler(this.updateScrollbar, 500, this));
    },
