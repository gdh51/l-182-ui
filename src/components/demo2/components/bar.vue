<template>
    <div
        class="custom_scrollbar-container"
        :class="bar.class"
    >
        <div
...
export default {
    ...
    computed: {
        ...
        spBarSize() {
            return (
                (this.$refs.bar &&
                    this.$refs.bar.getBoundingClientRect()[this.bar.rectSize]) ||
                0
            );
        },
    ...
    methods: {
        ...
        mousemoveHandler(e) {
            ...
            // 以滚动条长度为基计算鼠标的位移的占比
            const offsetPercentage = (e[this.bar.client] - this.clickPosition) / this.spBarSize;

            // 由于滚动条与滚动条槽的比等于视窗比上整个数据视图，所以可以换算为视窗应该移动的距离
            // 这里其实有个小问题，但不会出现问题的原因是scroll超过自己活动范围时，不会有影响
            this.viewElement[this.bar.scrollOffset] = this.viewPosition + offsetPercentage * this.viewSize;
        },
