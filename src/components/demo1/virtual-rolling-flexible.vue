...
export default {
    ...
    computed: {
        ...
        startIndex() {
            ...
            return this.positionInfo.find(
                item => (item.bottom >= this.topOffset)
            ).uuid;
        },
        ...
        // 可视区域下方预渲染的个数
        belowCount () {
            return Math.min(this.positionInfo.length - this.endIndex, this.prevRenderCount);
        }
    },
    ...
    methods: {
        ...
        correctLastTickNodeBottom (lastTickNodeBottom) {
            ...
            const LAST_NODE_BOTTOM = this.positionInfo[LAST_NODE_UUID >= 0 ? LAST_NODE_UUID : 0].bottom;

            // 防止0节点计算，因为0节点没有上一个节点
            // 如果上一个节点与当前节点应该继承的下底上出现断层，则要修复这个断层
            if (LAST_NODE_UUID !== -1 && LAST_NODE_BOTTOM !== lastTickNodeBottom) {
    ...
    updated() {
        ...
        if (this.alreadyRenderCount !== 100) {
            ...
            this.positionInfo
                .slice(+ CUR_RENDER_DOM_ITEMS[CUR_RENDER_DOM_ITEMS.length - 1].dataset.uuid + 1)
